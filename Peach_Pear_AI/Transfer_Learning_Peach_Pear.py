...
json_file = open(apple_kiwi_banana_orange_ai_path + '/Apple_Kiwi_Banana_Orange_AI_Architecture_No_Top.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
base_model = keras.models.model_from_json(loaded_model_json)

# # load weights into new model
base_model.load_weights(apple_kiwi_banana_orange_ai_path + "/Apple_Kiwi_Banana_Orange_AI_Weights.h5", by_name=True)

model = keras.Sequential()
model.add(base_model)
model.add(keras.layers.Flatten())
model.add(keras.layers.Dense(512, activation='relu'))
model.add(keras.layers.BatchNormalization(name="BatchNormalization_new_model"))
